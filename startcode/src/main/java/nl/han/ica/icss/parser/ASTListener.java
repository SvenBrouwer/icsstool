...
//    }
    ...
    public void enterOperation(ICSSParser.OperationContext ctx) {
        ...
        if (ctx.getChildCount() == 1) {
            if (ctx.colorliteral() != null) {
                stack.push(new ColorLiteral(ctx.getText()));
            } else if (ctx.pixelliteral() != null) {
    ...
    public void exitOperation(ICSSParser.OperationContext ctx) {
        ...
        if (ctx.getChildCount() == 1) {
            ...
            stack.peek().addChild(expression);
        }
        if (ctx.getChildCount() == 3) {
//            ASTNode right = stack.pop();
//            ASTNode operation = stack.pop();
